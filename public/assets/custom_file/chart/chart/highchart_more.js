	...
	i(e, "Extensions/Pane/PaneComposition.js", [e["Core/Utilities.js"]], function(t) {
		...
		class f extends a {
			...
			drawDataLabels() {
				...
				if (h) {
					...
					if (s.enabled)
						for (t = n; t--;)(e = a[t]) && (e.dataLabels = [e.dataLabelUpper, e.dataLabel].filter(function(t) {
							return !!t
						}));
				...
				for (let S of f) {
					...
					if (void 0 !== S.plotY) {
						...
						if (P) {
							...
							i = r(S.highPlot, t), s = r(S.lowPlot, t), w = [
								["M", r(T - (g = "string" == typeof P && /%$/.test(P) ? u * parseFloat(P) / 100 : Number(P) / 2)), i],
								["L", r(T + g), i],
								["M", r(T - g), s],
								["L", r(T + g), s]
							], S.whiskers[f]({
						...
						w = [
							["M", d, o = r(S.medianPlot, S.medianShape.strokeWidth())],
							["L", c, o]
						], S.medianShape[f]({
							d: w
						})
		...
		return class {
			...
			renderRange(t) {
				let e = this.ranges[0],
					...
					P = n.styledMode,
					S = g || "left" === u ? -c : c;
				"center" === u && (S = 0, s.connectorDistance = 0, t.labelAttribs.align = "center"), l.bubbleItems.push(n.circle(m, p + x, d).attr(P ? {} : t.bubbleAttribs).addClass((P ? "highcharts-color-" + r.colorIndex + " " : "") + "highcharts-bubble-legend-symbol " + (s.className || "")).add(this.legendItem.symbol)), l.connectors.push(n.path(n.crispLine([
					["M", m, y],
					["L", m + S, y]
		...
		function u(t) {
			let e = t.allItems,
				...
				l = 0;
			for (n = 0; n < s; n++)
				if (a = e[n].legendItem || {}, r = (e[n + 1] || {}).legendItem || {}, a.labelHeight && (e[n].itemHeight = a.labelHeight), e[n] === e[s - 1] || a.y !== r.y) {
					for (i.push({
							height: 0
		...
		function S() {
			...
			}), h && a > 0 && !this.logarithmic && (n -= t, l *= (t + Math.max(0, r) - Math.min(n, t)) / t, [
				["min", "userMin", r],
				["max", "userMax", n]
			].forEach(t => {
		...
		class M extends d {
			...
			getRadii() {
				let t = this.zData,
					e = this.yData,
					i = [],
					s, o, a, r = this.chart.bubbleZExtremes,
					{
						minPxSize: n,
						maxPxSize: l
					} = this.getPxExtremes();
		...
		class g extends a {
			...
			afterColumnTranslate() {
				...
				this.points.forEach(g => {
					...
					if (d(y) && d(m)) {
						if (g.plotHigh = h(y, -p, p), g.plotLow = h(m, -p, p), s = g.plotHigh, Math.abs(t = u(g.rectPlotY, g.plotY) - g.plotHigh) < b ? (e = b - t, t += e, s -= e / 2) : t < 0 && (t *= -1, s -= t), l && this.polar) i = g.barX + r, g.shapeType = "arc", g.shapeArgs = this.polar.arc(s + t, s, i, i + g.pointWidth);
						else {
							f.height = t, f.y = s;
							let {
								x: e = 0,
		...
		class h extends t {
			...
			drawDataLabels() {
				...
				if (a)
					for (let e of (a.prototype.drawDataLabels.call(this), this.points)) e.y = e[t]
			}
			toYData(t) {
				return [t.low, t.high]
			}
		...
		class c {
			...
			initPositions() {
				...
				if (h(t))
					for (let e of (t.call(this), this.nodes)) l(e.prevX) || (e.prevX = e.plotX), l(e.prevY) || (e.prevY = e.plotY), e.dispX = 0, e.dispY = 0;
			...
			setCircularPositions() {
				...
				if (l.length)
					for (let t of i) - 1 === l.indexOf(t) && l.push(t);
				else l = i;
				...
				let t = [],
					...
					p = (t, e) => {
						let {
							x: a,
							y: n
						} = e, l = (s.getRotationOfChar(t) - 90) * i, h = Math.cos(l), p = Math.sin(l);
						return [
							[a - r * h, n - r * p],
							[a + o * h, n + o * p]
						]
		...
		class A extends f {
			...
			placeBubbles(t) {
				let e = this.checkOverlap,
					...
					r = 0,
					n, l = [],
					h, p = t.sort((t, e) => e[2] - t[2]);
				if (p.length) {
					if (s.push([
							[0, 0, p[0][2], p[0][3], p[0][4]]
						]), p.length > 1)
						for (s.push([
								[0, 0 - p[1][2] - p[0][2], p[1][2], p[1][3], p[1][4]]
							]), h = 2; h < p.length; h++) p[h][2] = p[h][2] || 1, e(n = i(s[o][a], s[o - 1][r], p[h]), s[o][0]) ? (s.push([]), r = 0, s[o + 1].push(i(s[o][a], s[o][0], p[h])), o++, a = 0) : o > 1 && s[o - 1][r + 1] && e(n, s[o - 1][r + 1]) ? (r++, s[o].push(i(s[o][a], s[o - 1][r], p[h])), a++) : (a++, s[o].push(n));
		...
		return function(e) {
			...
			function S(t, e, i) {
				...
				return void 0 === i && (i = this.horiz ? 0 : this.center && -this.center[3] / 2), i && (l += i), this.isCircular || void 0 !== e ? ((h = this.chart.renderer.symbols.arc(a + s[0], r + s[1], l, l, {
					...
					open: !0,
					innerR: 0
				})).xBounds = [a + s[0]], h.yBounds = [r + s[1] - l]) : (n = this.postTranslate(this.angleRad, l), h = [
					["M", this.center[0] + o.plotLeft, this.center[1] + o.plotTop],
					["L", n.x, n.y]
				]), h
			}
			...
			function L(t, e, i) {
				let s = this.chart,
					...
					d = /%$/,
					c = this.isCircular,
					u, g, b, m, y, x, P = f(o(i.outerRadius), n),
					S = o(i.innerRadius),
			...
			function C(t) {
				let e = this.pane.center,
					...
					c = e[3] / 2,
					u = t.value,
					g, f, b, y, x, P, S, M, L, C = this.getPosition(u),
					k = C.x,
					v = C.y;
				if (d && (u = (M = this.getCrosshairPosition(t, l, h))[0], k = M[1], v = M[2]), this.isCircular) f = Math.sqrt(Math.pow(k - l, 2) + Math.pow(v - h, 2)), b = "string" == typeof r ? m(r, 1) : r / f, y = "string" == typeof n ? m(n, 1) : n / f, e && c && (b < (g = c / f) && (b = g), y < g && (y = g)), L = [
					["M", l + b * (k - l), h - b * (h - v)],
					["L", k - (1 - y) * (k - l), v + (1 - y) * (h - v)]
				];
				else if ((u = this.translate(u)) && (u < 0 || u > p) && (u = 0), "circle" === this.options.gridLineInterpolation) L = this.getLinePath(0, u, c);
		...
		function T(t, e, s, o, a, r) {
			...
			if (p.polar) {
				if (n = e.rectPlotX / Math.PI * 180, p.inverted) this.forceDL = p.isInsidePlot(e.plotX, e.plotY), d && e.shapeArgs ? (l = e.shapeArgs, a = c(a, {
					...
					x: e.tooltipPos[0],
					y: e.tooltipPos[1]
				})), o.align = u(o.align, "center"), o.verticalAlign = u(o.verticalAlign, "middle");
				else {
					var g;
					let t, e;
		...
		function N() {
			let t = this.options,
				...
				M, L, C, k, v, A, w, T;
			if (s.isRadial)
				for (m = (g = this.points).length, y = o.translate(o.min), x = o.translate(o.max), c = t.threshold || 0, i.inverted && d(c) && h(u = o.translate(c)) && (u < 0 ? u = 0 : u > p && (u = p), this.translatedThreshold = u + l); m--;) {
		...
		class y extends a {
			...
			setStackedPoints(t) {
				...
				if (t.stacking && i && this.reserveSpace()) {
					...
					let t = i[o];
					if (t)
						for (let i = 0; i < r; i++)(!t[y = a[i]] || P) && (t[y] = {
							negTotal: 0,
							posTotal: 0,
							stackTotal: 0,
							threshold: 0,
		...
		return y.defaultOptions = g(a.defaultOptions, o), y.compose = i.compose, c(y.prototype, {
			...
			let {
				...
				points: e,
				yAxis: i
			} = this, s = b(t.minPointLength, 5), o = s / 2, a = t.threshold || 0, r = t.stacking, n = i.waterfall.stacks[this.stackKey], l = a, h = a, p, f, y, x;
			for (let t = 0; t < e.length; t++) {
				let b = e[t],
